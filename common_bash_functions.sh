...
function info_message
{
    ...
    echo "$MODNAME:info:$_message" >&2
}

function timed_info_message
{
    local _message="$1"

    echo "`date +'%Y-%m-%d %H:%M:%S'` $USER $_message"
}
...
function pid_exists
{
    ...
    # https://stackoverflow.com/a/5207048/1029379
...
function pif_exists     # pid-in-file
{
    local _pid_filepath="$1"
    local _pid=0

    if [[ -f $_pid_filepath ]]
    then
        _pid=$(cat $_pid_filepath)
        [[ -z $_pid ]] && { _pid=0; }
    fi
...
function pif_terminate  # pid-in-file
{
    local _pid_filepath="$1"
    local _pid=0
...
function change_tz_iso_to_epoch_time
{
    local _tz="$1"
    local _iso_time="$2"
    local _epoch_time=$(TZ=$_tz date --date="$_iso_time" +%s)
...
function current_iso_datetime_str
{
    echo "`date +%Y-%m-%dT%H:%M:%S.%N`"
}
